class App extends React.Component {
  ...
  getQuestion = () => {
    ...
    fetch("http://jservice.io/api/random") //GET request by default
      ...
      .then((response) => {
        //.then is a fuction that the output returns a promise " .then "this is the fuction to call".
  ...
  isRevealed = () => {
    //fucntion to reveal the question after it has been fetched.
    this.setState({ isRevealed: !this.state.isRevealed });
  };

  increasetItem = () => {
    // Function to increase using the "Increase" button
    this.setState({ score: this.state.score + 1 });
  };
  decreaseItem = () => {
    // Function to decrease using the "decrease" button
    this.setState({ score: this.state.score - 1 });
  };
  ...
  render() {
    return (
      <div>
        ...
        <div class="buttons">
          <button className="decr" onClick={this.decreaseItem}>
            {" "}
            Decrease
          </button>
          <button className="incr" onClick={this.increasetItem}>
            {" "}
            Increase{" "}
          </button>
          <button className="res" onClick={this.resetItems}>
            {" "}
            Reset{" "}
          </button>
        </div>
        <h2> Let's Play! </h2>
        <button className="getquest" onClick={this.getQuestion}>
          {" "}
          Get Question{" "}
        </button>
        <h2>
        ...
        <h3>
          {" "}
          Answer: <span>{this.state.clue}</span>{" "}
        </h3>
        <button className="reveal" onClick={this.isRevealed}>
          {" "}
          Click to Reaveal Question{" "}
        </button>
        {this.state.isRevealed ? (
          <h3>
            {" "}
            Question: <span>{this.state.answer}</span>{" "}
          </h3>
        ) : (
          ""
        )}
